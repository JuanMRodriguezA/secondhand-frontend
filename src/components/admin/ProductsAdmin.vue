    ...
    <div class="relative z-10 container mx-auto px-4 py-8 md:py-12">
      ...
      <div class="flex flex-col lg:flex-row gap-8">
        ...
        <main class="lg:w-3/4">
          ...
          <!-- Paginación abajo -->
          <div v-if="totalPages > 1" class="mt-8 flex flex-col sm:flex-row justify-center items-center gap-4">
            <div class="flex items-center gap-2">
              <button
                @click="prevPage"
                :disabled="currentPage === 1"
                class="px-4 py-2 rounded-full bg-white/80 border border-[#a74792]/20 disabled:opacity-30 flex items-center gap-1 hover:bg-white transition shadow-sm"
              >
                <i class="mdi mdi-chevron-left"></i> Anterior
              </button>
              <span class="text-[#2D1E3E] font-medium px-2">Pág. {{ currentPage }} de {{ totalPages }}</span>
              <button
                @click="nextPage"
            ...
            <!-- Selector de páginas (opcional) -->
            <select v-model="currentPage" class="px-3 py-2 border border-[#a74792]/20 rounded-lg text-sm bg-white">
...
// Computed para categorías y marcas
...
// Productos filtrados (misma lógica)
...
<style scoped>
/* Estas clases deben estar globales, pero se mantienen por si acaso */
